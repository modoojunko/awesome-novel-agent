...
> 本文件改写自获赠短篇流程包（oh-story 系 + 实测调整），按本仓口径适配：引用为部署后基座路径、已剔除外部体系引用。公开或商用前需原作者确认。


> 短篇是投出去过稿的，不是写完就算。这份文件管三件投稿层的事：按平台定基调、单独打磨导语、把付费点卡在断点上。题材招式仍按 `.claude/knowledge/short-genres/{题材}.md`，本文只叠加"投稿怎么写才收得掉"这一层。
