...
**触发入口：** ① setup 阶段讨论到文风时（主动给决策点）；② 作者任何 phase 说「修改文风设定/更新文风/重新蒸馏/按这个风格写」→ 立即进入本流程（不打断当前调度，蒸馏 DONE 后再回到原流程）。

**流程：**
1. 给作者三选一（话术示例）：
   ```
   "风格这块想怎么定？
    ① 蒸馏——给我任意文档（项目内任意路径的 .md/.txt，或直接聊天粘贴文章，
       6000-10000 字、风格纯净）；没有现成文档就用已归档正文（现成选择）
    ② 模板——直接用模板题材基线（默认文风，放弃/不使用当前蒸馏卡）
    ③ 暂缓——先不弄，随时说"修改文风设定"再回来"
   ```
2. **选 ①（蒸馏）**：
   - 收样本：**作者把待蒸馏文章放到项目根 `novel-samples/` 目录**（专用目录，不存在时 novel-agent 创建；类比 sandbox/ 推演目录）——该目录下的任意 .md/.txt 直接入 order；聊天粘贴文本由 novel-agent 落盘为 `.agent/task/style-sample.md` 再入 order；已归档正文（archives/）是现成选择之一，不限定
   - **项目外路径（如桌面/下载）→ 提示作者无权限读取，请把文档复制进项目内 `novel-samples/` 再蒸馏**——所有样本操作限定在项目根内
...
- **Loop Integration:**
  ...
  THINK:
    是否建议推演沙盘？← 二(推演沙盘评估逻辑)
    作者说"修改文风设定/更新文风/重新蒸馏/按这个风格写"（任何 phase）？→ 二(文风设定决策流程)
