...
2. **平台无关的适配器接口。**
   skill 只约定三件事：送检入口（本地命令或作者手动网页送检）、结果文件（逐片：起止位置/分数/文本）、判定阈值参数。来源包的 CDP/验证码/额度类脚本与逆向文档属绕过平台风控的工具，**不保留、不迁移、永不入库**（2026-08-31 已删除本地副本）；仓库只保留结果文件格式契约，对接方式由作者以合法途径自备（如官方 API 或手动网页送检）。
