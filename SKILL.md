...
- **可选工具（不进调度链，作者主动触发）**：作者说「扫个榜 / 看看什么火」进入 `short-scan`（市场选题）；说「拆这篇 / 分析这篇爆款」进入 `short-analyze`（对标拆解，产出 `analysis/{书名}/` 供写作阶段召回）——两个工具的 SOP 在平台 skills 目录下，按各自 SOP 执行，不写 order、不占 phase
